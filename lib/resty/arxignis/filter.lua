...
local function sanitize_base_url(url)
  ...
  if sanitized == '' then
    sanitized = DEFAULT_BASE_URL
  end
  return sanitized
end

local function encode_args(args)
  if ngx and ngx.encode_args then
...
local function read_request_body()
  ...
  if not file then
    if ngx.log then
      ngx.log(ngx.WARN, "Arxignis filter: unable to read temp body file: ", tostring(open_err))
    end
    return nil
  end
...
function filter.build_event_from_request(opts)
  ...
  if not ngx or not ngx.req then
    return nil, "ngx.req is not available"
  end
  ...
  if body then
    ...
    local content_length_header = headers["content-length"] or headers["Content-Length"]
  ...
  if opts.additional then
    event.additional = opts.additional
  end
